...
class SearchPostService {
  ...
  Future<List<Profile>?> searchProfile(SearchProfileModel model) async {
    ...
    if (model.query.isNotEmpty) {
      ...
      debugPrint(
        'Filtering for display_name containing: "$searchQuery" (in Dart)',
      );
    ...
    Map<ProfileType, Set<String>> validProfileIds = {
      ProfileType.architect: _getProfileIdsForType(
        result,
        ProfileType.architect,
      ),
      ProfileType.contractor: _getProfileIdsForType(
        result,
        ProfileType.contractor,
      ),
      ProfileType.constructionTeam: _getProfileIdsForType(
        result,
    ...
    if (needsSpecificFiltering[ProfileType.architect]!) {
      ...
      debugPrint(
        'After design style filtering: ${filteredArchitectIds.length} architects',
      );
    }
    ...
    if (needsSpecificFiltering[ProfileType.constructionTeam]!) {
      final serviceTypes =
          model.constructionTeamFilterModel!.serviceType
              ...
              .toList();
      debugPrint(
        'Filtering construction teams by service types: $serviceTypes',
      );
      ...
      for (final p in profileIdsTeam) {
        if (serviceTypes.contains(p['service_type']) &&
            validProfileIds[ProfileType.constructionTeam]!.contains(
              p['profile_id'],
            )) {
      ...
      debugPrint(
        'After service type filtering: ${filteredTeamIds.length} construction teams',
      );
    }
    ...
    if (needsSpecificFiltering[ProfileType.contractor]!) {
      ...
      for (final p in profileIdsContractor) {
        if (serviceTypes.contains(p['service_type']) &&
            validProfileIds[ProfileType.contractor]!.contains(
              p['profile_id'],
            )) {
      ...
      debugPrint(
        'After service type filtering: ${filteredContractorIds.length} contractors',
      );
    }
    ...
    if (needsSpecificFiltering[ProfileType.supplier]!) {
      final materialCategories =
          model.supplierFilterModel!.materialCategory
              ...
              .toList();
      debugPrint(
        'Filtering suppliers by material categories: $materialCategories',
      );
      ...
      debugPrint(
        'After material category filtering: ${filteredSupplierIds.length} suppliers',
      );
    }
